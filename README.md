...
```mermaid
flowchart TD
    A["用户输入书名或设定"] --> B{"必填参数齐全？"}
    B -->|否| C["追问题材、目标章数、每章字数、受众、风格、禁写内容"]
    C --> B
    B -->|是| D["初始化项目<br/>project.json + state.json"]
    D --> E["前置规划<br/>定位 / 创意 / 大纲 / 角色 / 开局 / 爽点账本"]
    E --> F["章节循环<br/>章节卡 / 正文 / 记忆"]
    F --> G{"达到目标章数？"}
    G -->|否| F
    G -->|是| H["全稿完成"]
```
...
每个规划阶段都会先生成候选产物，确认后才进入下一步。

```mermaid
flowchart TD
    A["市场定位<br/>读者承诺 / 类型赛道 / 爽点禁区"] --> B["创意孵化<br/>主角 / 核心事件 / 失败代价 / 隐藏危机"]
    B --> C["故事大纲<br/>阶段结构 / 冲突升级 / 兑现系统 / 滚动目录"]
    C --> D["角色设计<br/>目标 / 缺陷 / 恐惧 / 秘密 / 反派压迫方式"]
    D --> E["开局钩子<br/>前三章压力 / 行动 / 反转 / 留钩"]
    E --> F["读者参与账本<br/>情绪曲线 / 爽点台账 / 悬念揭示 / 章末追问"]
    F --> G["进入第 1 章章节卡"]
```

### 章节循环与质量闸门

章节卡、正文和五章复盘都可能要求修订；确认和校验通过后才会推进状态。

```mermaid
flowchart TD
    A["读取 state.json<br/>确认没有待确认产物"] --> B["生成 chapter_NNN_card.md"]
    B --> C["set_pending.py<br/>校验章节卡并打开确认闸门"]
    C --> D{"用户确认章节卡？"}
    D -->|拒绝 / 修订| B
    D -->|确认| E["confirm_artifact.py<br/>推进到正文阶段"]
    E --> F["生成 chapter_NNN.md<br/>写真实正文，不写摘要"]
    F --> G["校验正文<br/>Prose 字数 / 事件覆盖 / 因果链 / 结尾钩子 / 流程词"]
    G --> H{"用户确认正文？"}
    H -->|拒绝 / 修订| F
    H -->|确认| I["生成 memory_NNN.md<br/>事件 / 人物状态 / 世界事实 / 未解悬念 / 下一章承接"]
    I --> J{"是否满 5 章？"}
    J -->|否| K["next_chapter + 1"]
    K --> A
    J -->|是| L["生成 block_review_XXX_YYY.md<br/>复盘节奏、兑现、伏笔、人物和逻辑"]
    L --> M{"存在 critical/high 问题？"}
    M -->|是| N["进入 revision_required<br/>先修问题章节"]
    N --> A
    M -->|否| K
```
