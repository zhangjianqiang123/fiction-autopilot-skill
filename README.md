...
```mermaid
flowchart TD
    A[用户输入] --> B["Phase 0 项目初始化<br/>补齐必填参数<br/>创建 project/state<br/>检测断点状态"]
    B --> C["Phase 1-2 故事规划<br/>市场定位 → 创意孵化 → 大纲 → 角色<br/>开局钩子 → 读者爽点账本"]
    C --> D["Phase 3 章节循环<br/>章节卡 → 用户确认 → 正文 → 用户确认<br/>记忆更新 → 下一章"]
    D --> E["Phase 4 质量复盘<br/>每 5 章检查节奏、兑现、伏笔、人物和逻辑<br/>critical/high 问题进入 revision 状态"]
    E --> F[全稿完成]
```
